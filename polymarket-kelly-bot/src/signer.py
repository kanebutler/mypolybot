"""
Signer Module - EIP-712 Order Signing for Polymarket

Signs orders and authentication messages using EIP-712 structured data.

Example:
    from src.signer import OrderSigner, Order
    signer = OrderSigner(private_key)
    signed = signer.sign_order(Order(token_id="...", price=0.65, size=10, side="BUY", maker="0x..."))
"""
...
class OrderSigner:
    ...
    def sign_auth_message(self, timestamp: Optional[str] = None, nonce: int = 0) -> str:
        ...
        signable = encode_typed_data(
            domain_data=self.DOMAIN, message_types=auth_types, message_data=msg
        )
