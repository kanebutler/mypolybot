...
PBKDF2 key derivation + Fernet symmetric encryption.

Example:
    from src.crypto import KeyManager
    manager = KeyManager()
    manager.encrypt_and_save(private_key, password, "credentials/key.enc")
    key = manager.load_and_decrypt(password, "credentials/key.enc")
"""
...
class KeyManager:
    ...
    def decrypt(self, encrypted_data: dict, password: str) -> str:
        try:
            ...
            decrypted = cipher.decrypt(
                base64.urlsafe_b64decode(encrypted_data["encrypted"])
            )
            return f"0x{decrypted.decode()}"
        except InvalidToken:
    ...
    def encrypt_and_save(self, private_key: str, password: str, filepath: str) -> Path:
        data = self.encrypt(private_key, password)
        path = Path(filepath)
    ...
    def load_and_decrypt(self, password: str, filepath: str) -> str:
        path = Path(filepath)
        if not path.exists():
            raise FileNotFoundError(f"Encrypted key not found: {filepath}")
        with open(path) as f:
            data = json.load(f)
